...
CACHE_DIR = 'database/tmp/bottleneck'
INPUT_DATA = 'database/flower_photos'
...
def get_image_path(image_lists, image_dir, label_name, index, category):
    '''
    通过类别名称、所属数据集和图片编号获取一张图片的名称
    :param image_lists: 
    :param image_dir: 
    :param label_name: 
    :param index:  需要获取的图片的编号
    :param category:  指定需要获取的图片属于训练集、测试集还是验证集
    :return: 
    '''
    #获取给定类别中所有图片的信息
...
def get_bottleneck_path(image_lists, label_name, index, category):
    '''
    通过类别名称，所属数据集和图片编号获取经过Inception-v3处理后的特征向量文件地址
    :param image_lists: 
    :param label_name: 
    :param index: 
    :param category: 
...
def run_bottleneck_on_image(sess, image_data, image_data_tensor, boottleneck_tensor):
    '''
    使用加载的训练好的Inception-v3模型处理一张图片，得到这个图片的特征向量
    :param sess: 
    :param image_data: 
    :param image_data_tensor: 
    :param boottleneck_tensor: 
    :return: 
    '''
    bottleneck_values = sess.run(boottleneck_tensor,
                                 {image_data_tensor:image_data})
    #经过卷积神经网络处理的结果是一个四维数组，需要将这个四维数组压缩成一个特征向量（一维数组）
...
def get_or_create_bottleneck(sess, image_lists, label_name, index,
    ...
    #如果这个特征向量文件不存在就用inception-v3模型来计算特征向量，并将计算结果存入文件
...
def get_random_cached_bottlenecks(sess, n_classes, image_lists, how_many, category,
                                  jpeg_data_tensor, bottle_tensor):
    '''
...
def get_test_bottlenecks(sess, image_lists, n_classes, jpeg_data_tensor, bottleneck_tensor):
    ...
    #枚举所有的类别和每个类别中的测试图片
    for lable_index, label_name in enumerate(label_name_list):
        category = 'testing'
        for index, unused_base_name in enumerate(image_lists[label_name][category]):
            #通过Inception-v3模型计算图片对应的特征向量，并将其加入最终的数据列表
...
def main():
    ...
    #定义新的神经网络的输入，这个输入就是新的图片经过inception-v3模型前向传播达到瓶颈层的节点取值，
    #可以将这个过程类似理解成一种特征提取。
    ...
    with tf.Session() as sess:
        ...
        for i in range(STEPS):
            ...
            if i % 1 == 0 or i + 1 == STEPS:
                ...
                print('Step %d: Validation accuracy on random sampled %d examples = %.2f%%'%(i, BATCH, validation_accuracy * 100))
